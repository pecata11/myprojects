...
//  FileCryptor
//
//  Created by Petko Yanakiev on 3/20/12.
...
//Added by Petko at 11.04.2012.
-(void) compareSessionIds:(NSDictionary*) serverHeaders;
- (void)filterResponseHeaders:(NSHTTPURLResponse *)response;
@end
...
+(id)requestToURL:(NSURL*)url delegate:(id)del endSelector:(SEL)anEndSelector contextInfo:(id) context:username:(id)uname:password:(id)pass{
...
+(id)requestURL:(NSURL*)url delegate:(id)del endSelector:(SEL)anEndSelector contextInfo:(id) context:(NSString*)uname:(NSString*)pass{
...
- (void)sendRequest:(NSMutableURLRequest *)req:(NSString*)sess_id {
    
    [req setCachePolicy:NSURLRequestReloadIgnoringLocalCacheData];
    [req setValue:sess_id forHTTPHeaderField:@"SESSION_ID"];
    ...
    else if (_synchronous) 
    {
        ...
        NSError *err = nil;
        dataResponse=[NSURLConnection sendSynchronousRequest:req returningResponse:&response error:&err];
        //NSLog(@"The response data is:%@",dataResponse);
        //NSLog(@"The response isss %@",response);
        
        [self setResponseData:(NSMutableData*)dataResponse];
...
- (FMWebDavRequest*)putData:(NSData*)data:(NSString*)sess_id {
    ...
    [self sendRequest:req:sess_id];
    
    return self;
}

- (FMWebDavRequest*)get:(NSString*)session_id {
    
    if (!_endSelector) {
        _endSelector = @selector(requestDidGet:);
    }
    NSLog(@"The requets url is:%@",_url);
    NSMutableURLRequest *req = [NSMutableURLRequest requestWithURL:_url];
    [req setCachePolicy:NSURLRequestReloadIgnoringLocalCacheData];
    [req setTimeoutInterval:60 * 5];
    [req setHTTPMethod:@"GET"];
   
    [self sendRequest:req:session_id];
...
- (FMWebDavRequest*)fetchDirectoryListing {
    //NSLog(@"fetchDirectoryListing start");
...
- (FMWebDavRequest*)fetchDirectoryListingWithDepth:(NSUInteger)depth extraToPropfind:(NSString*)extra {
   ...
   // NSLog(@"The url length is:%@",_uriLength);
    ...
    //[req setValue:@"Hello Request" forHTTPHeaderField:@"SessionKey"];
    [self sendRequest:req:@""];
...
- (NSArray*)directoryListing {
    ...
    for (NSDictionary *dict in [self directoryListingWithAttributes]) {
            //NSLog(@"The object is:%@",dict);
        if ([dict objectForKey:@"href"]) {
            //NSLog(@"The object for key href is:%@",[dict objectForKey:@"href"]);
...
- (void)logRequest:(NSURLRequest *)request {
    ...
    writeAlways = YES;  // YES if HTTP traffic is wanted, NO if not
#endif
...
-(void) compareSessionIds:(NSDictionary*) serverHeaders{
    ...
    NSString *clientSession_id = nil;
    
    if([fm fileExistsAtPath:serializedHeaderPath]){
        
        NSDictionary *clientDeserializedHeaders = [NSKeyedUnarchiver unarchiveObjectWithFile:serializedHeaderPath];
        clientSession_id = [clientDeserializedHeaders objectForKey:@"session_id"];
    }
    else{ 
        clientSession_id = @"";
    }
    
    NSString *serverSession_id = nil;
    serverSession_id = [serverHeaders objectForKey:@"session_id"];
    
    if(![serverSession_id isEqualToString:clientSession_id])
    {
        [NSKeyedArchiver archiveRootObject:serverHeaders toFile:[docsFolder stringByAppendingPathComponent:SERIALIZED_HEADERS]];
    }
    [fm release];
}
...
- (void)filterResponseHeaders:(NSHTTPURLResponse *)response{
    ...
    NSMutableDictionary *filteredHeaders = [[[NSMutableDictionary alloc]init]autorelease]; 
    for(id key in headers)
    {
        id obj = [headers objectForKey: key];
        [filteredHeaders setObject: obj forKey: [key lowercaseString]];
    }
    
    [self compareSessionIds:filteredHeaders];
}

- (void)logResponse:(NSHTTPURLResponse *)response {
    
    BOOL writeAlways = YES;
...
- (void)connection:(NSURLConnection *)connection didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    ...
    if ([[[challenge protectionSpace] authenticationMethod] isEqualToString:NSURLAuthenticationMethodServerTrust]) 
    {
        ...
        NSLog(@"Accepting server trust challenge for '%@'", [[challenge protectionSpace] host]);
        ...
        NSLog(@"The password didn't work!");
        _rlSynchronous = NO;
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)URLresponse {
    ...
    [self logResponse:httpResponse];
    //added by petko at:7.05.2012.
    [self filterResponseHeaders:httpResponse];
...
//#pragma message FIXME: go ahead and do this bit.
            ...
            //NSLog(@"The content type is:%@",_xmlChars);
        }
