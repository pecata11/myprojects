...
@synthesize selectedTile, highlightedTile, transitioning,navController;

- (id)initWithFrame :(CGRect)frame logic :(KalLogic *)theLogic delegate:(id<KalViewDelegate>)theDelegate :(UINavigationController*) navigationControler
{
...
{
  // This sets the cell background!!!
  [[UIImage imageNamed:@"white.png"] drawInRect:rect];
  //[[UIColor colorWithRed:0.f green:0.f blue:0.f alpha:1.f] setFill];
  //CGRect line;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
  ...
  if ([hitView isKindOfClass:[KalTileView class]]) 
  {
    ...
    {    
        KalDate *dd = tile.date;   
        NSString* str= [logic selectedMonthNameAndYear];
        
        if(tile.marked)
        {
            CalendarBirthdaysViewController *mBirthdayVC =[[CalendarBirthdaysViewController alloc]initWithUserData:dd.year:dd.month:dd.day:str];
            [navController pushViewController:mBirthdayVC animated:YES];
            [mBirthdayVC release];
            tile.selected=FALSE;
        }
        self.selectedTile = tile;
